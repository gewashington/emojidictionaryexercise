...
class ViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Emoji Table")!
        let row = indexPath.row
        let emojiInRow = emojis[row]
        cell.textLabel?.text = "\(emojiInRow.symbol) - \(emojiInRow.name)"
        cell.detailTextLabel?.text = emojiInRow.description
        return cell
